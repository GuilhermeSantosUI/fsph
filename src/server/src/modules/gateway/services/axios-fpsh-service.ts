import { fpshApi } from '@shared/infra/http/axios-fsph';
import dayjs from 'dayjs';

import {
  DonationPlace,
  GetAvailableCitiesApiResponse,
  TimeBlock,
  TimeBlockDate,
} from '../dtos/fpsh-dto';
import {
...
export class AxiosFpshGateway implements FpshGateway {
  ...
  async getDonorProfile(cpf: string): Promise<unknown> {
    try {
      const { data } = await fpshApi.get(
        `/apiagendamento/doador/getinfo/${cpf}`
      );
  ...
  async getAllBlocks(params: GetAllBlocksParams): Promise<TimeBlock[]> {
    const [pi, pm, pc] = this.permTuple(params.tipo_atendimento);
  ...
  async makeCampaignAppointment(
    ...
    form.append(
      ...
      this.toYMD(body.responsibleBirthDate)
    );
