...
export default function AgendarCampanhaConfirm() {
  const router = useRouter();
  const params = useLocalSearchParams();

  const name = (params.name as string) ?? "Ana Beatriz";
  const location = (params.location as string) ?? "Riomar";
  const campaign = (params.campaign as string) ?? "Julho Vermelho";

  const [successVisible, setSuccessVisible] = useState(false);

  const handleConfirm = () => {
    // aqui você pode chamar a API para confirmar o agendamento de campanha
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.content}>
        <Text style={styles.subtitle}>confirme os dados da sua doação para campanha:</Text>
        ...
        <View style={styles.card}>
          <Text style={styles.cardLabel}>Campanha:</Text>
          <View style={styles.fieldValue}>
            <Text style={styles.fieldText}>{campaign}</Text>
          </View>
        </View>
      </View>
      ...
      <View style={styles.bottomArea}>
        <View style={styles.bottomInner}>
          <TouchableOpacity style={styles.confirmButton} onPress={handleConfirm} accessibilityLabel="Confirmar agendamento de campanha">
...
const styles = StyleSheet.create({
  ...
  subtitle: { textAlign: "center", fontStyle: "italic", color: "#9A9A9A", marginBottom: 12, fontSize: 20 },
  ...
  confirmText: { color: "#ffffffff", fontWeight: "700", fontSize: 16 },
  // success modal
  successOverlay: {
    flex: 1,
